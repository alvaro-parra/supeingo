...
(function (root, factory) {
  ...
  // options:
  //   title (string, default "Sopa de letras") — única seña en la página
  //   includeSolution (bool, default true)
  //   now (Date, opcional — para tests deterministas)
  //
  // Filename fijo `sopa-YYYYMMDD.pdf` y título sin categoría: la profe
  // imprime varias fichas con mezclas distintas y la categoría en el
  // nombre/título daba pistas inconsistentes (multicategoría no encaja
  // en un solo slug). Mejor neutro.
  function buildPdfSpec(board, options) {
    ...
    return {
      meta: {
        ...
        filename: "sopa-" + todayYmd(now) + ".pdf",
      },
      pages: pages,
    };
  }
